...
import { CROSS_REPO_DISPLAY_LIMIT, PER_REPO_FETCH_LIMIT } from '../../../../src/shared/work-items'
import type {
...
import { resolveComposerBranchSelection } from '../../../../src/shared/composer-branch-selection'
import type {
  BaseRefSearchResult,
...
export default function MobileTasksScreen() {
  ...
  const workspaceAgentOptions = useMemo<PickerOption<WorkspaceAgentChoice>[]>(() => {
    const availableAgents =
      workspaceDetectedAgentIds === null
        ? new Set<TuiAgent>(TUI_AGENT_AUTO_PICK_ORDER)
        : new Set<TuiAgent>(
            TUI_AGENT_AUTO_PICK_ORDER.filter((agent) => workspaceDetectedAgentIds.has(agent))
          )
    if (
      workspaceAgent &&
    ...
    const agents = TUI_AGENT_AUTO_PICK_ORDER.filter((agent) => availableAgents.has(agent))
    return [
      ...agents.map((agent) => ({
        value: agent,
  ...
  const mergeHostedReview = useCallback(
    async (
      ...
      if (item.provider === 'github' && isGitHubPrMergeBlocked(item)) {
        setError('GitHub reports merge conflicts. Open the GitHub merge box to continue.')
        return
      }
      setMutatingStatus(true)
      setError('')
  ...
  const renderDetailComment = (
    ...
    >
      ...
      {actionItem?.provider === 'github' && detailPayload?.provider === 'github' ? (
                      ...
                      {projectRowDetail.comments.length === 0 ? (
                        ...
                        groupDetailComments(projectRowDetail.comments).map((group) => {
                          ...
                          const renderProjectComment = (
                            ...
                            const isEditingComment = projectEditingCommentId === commentId
                            return (
                              <View
                                key={commentId}
                                style={[
                                  ...
                                  <>
                                    ...
                                    <View style={styles.inlineActionRow}>
                                      ...
                                      <Pressable
                                        ...
                                        onPress={() =>
                                          void replyToProjectGitHubComment(projectRowItem, comment)
                                        }
                                      >
                                        <Text style={styles.inlineSaveText}>Reply</Text>
                                      </Pressable>
              ...
              projectRowItem.content.state !== 'MERGED' ? (
                <>
                  ...
                  <View style={styles.linearStatesBlock}>
                    <Text style={styles.linearStatesTitle}>Merge</Text>
                    {(['squash', 'merge', 'rebase'] as const).map((method) => (
                      <Pressable
                        ...
                        onPress={() =>
                          setPendingProjectGitHubMerge({ row: projectRowItem, method })
                        }
                      >
                    ...
                    {projectRowItem.content.url ? (
                      ...
                      >
                        <ExternalLink size={16} color={colors.textPrimary} />
                        <Text style={styles.actionText}>Open GitHub merge box</Text>
                      </Pressable>
                    ) : null}
                    {!projectRowHostedRepo ? (
                      <Text style={styles.emptyInlineText}>
            ...
            <View style={styles.detailGroup}>
              {detailLoading ? (
                ...
                <>
                  ...
                  {detailPayload.provider === 'github' && detailPayload.files.length > 0 ? (
                    <View style={styles.detailSection}>
                      <Text style={styles.detailSectionTitle}>Changed files</Text>
                  ...
                  <View style={styles.linearStatesBlock}>
                    ...
                    {(['squash', 'merge', 'rebase'] as const).map((method) => (
                      <Pressable
                        ...
                        onPress={() =>
                          setPendingGitHubMerge({
                            item: actionItem as Extract<TaskItem, { provider: 'github' }>,
                            method
                          })
                        }
                      >
                  ...
                  <View style={styles.linearStatesBlock}>
                    ...
                    {(['merge', 'squash', 'rebase'] as const).map((method) => (
                      <Pressable
                        ...
                        style={styles.actionRow}
                        disabled={mutatingStatus}
                        onPress={() => void mergeHostedReview(actionItem, method)}
                      >
                        <GitBranch size={16} color={colors.textPrimary} />
                        <Text style={styles.actionText}>
                          {method === 'squash'
